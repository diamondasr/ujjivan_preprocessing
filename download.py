...
from data_utils import init_system, download_transcriptions
from data_utils import download_audio_json, convert_single_file
...
argument_parser.add_argument('-lang',
                       type=str,
                       help='the lang id used in url to access azure blob storage, eg: for tamil- ta', required=True)

argument_parser.add_argument('-source_mp3_dir',
                       type=str,
                       help='the source mp3 directory containing audio (.mp3) files', required=True)
...
argument_parser.add_argument('-transliteration_map_file_path',
                       type=str,
                       help='path for transliteration mapping file')
...
audio_url = audio_source + language_code
text_url = text_source + language_code
...
audio_json = os.getcwd() + "/data/" + language_code +"/audio.json"
...
# initialize system
init_system(language_code , destination_wav_directory)

# download transcriptions json and then creates a list of words and then runs g2p to create final lexicon file
download_transcriptions(text_url, transcription_filepath, temp_lexicon_path,\
...
audio_json = download_audio_json(audio_url,audio_json)
...
try:
    if speaker_id != -1:
        ...
        for row in tqdm(speaker_rows):
            ...
            if extension_valid:
                        destination_mp3_path = convert_single_file(row, downloaded_audio_count, speaker_id,\
